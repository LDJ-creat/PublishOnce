...
async function initPlatforms() {
    try {
        ...
        console.log('数据库连接成功');
        const existingPlatforms = await Platform_1.Platform.countDocuments();
        if (existingPlatforms > 0) {
            console.log(`数据库中已存在 ${existingPlatforms} 个平台配置，跳过初始化`);
            return;
        }
        const result = await Platform_1.Platform.insertMany(defaultPlatforms);
...
async function updatePlatforms() {
    try {
        ...
        for (const platformData of defaultPlatforms) {
            const existingPlatform = await Platform_1.Platform.findOne({ name: platformData.name });
            if (existingPlatform) {
                await Platform_1.Platform.findOneAndUpdate({ name: platformData.name }, {
                    ...platformData,
                    updatedAt: new Date()
                }, { new: true });
                updatedCount++;
                ...
                await Platform_1.Platform.create(platformData);
                createdCount++;
...
//# sourceMappingURL=initPlatforms.js.map